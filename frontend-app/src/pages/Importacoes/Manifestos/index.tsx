...
export function ManifestosImport() {
    ...
    const proximoPasso = () => setPassoAtual((prev) => Math.min(prev + 1, STEPS.length));
    const voltarPasso = () => setPassoAtual((prev) => Math.max(prev - 1, 1));

    // Sem arquivo importado não há o que mapear nos passos seguintes.
    const podeContinuar = passoAtual === 1 ? importacao !== null : passoAtual < STEPS.length;
        ...
        <div className="importacao-container">
            ...
            <div className="step-footer">
                ...
                <button
                    ...
                    title={passoAtual === 1 && !importacao ? 'Envie um arquivo para continuar' : undefined}
                >
                    Continuar
                    <ArrowRight size={20} />
                </button>
